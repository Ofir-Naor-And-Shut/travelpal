...
| **OpenStreetMap / basemap tile providers** | The four map backgrounds (Streets = `tile.openstreetmap.org`, Minimal = Esri light-gray canvas, Terrain = OpenTopoMap, Satellite = Esri World Imagery) in the Leaflet map, plus the PDF export's static map snapshot (`tile.openstreetmap.org`). | Each is free only because it's meant for light/non-commercial use — **not just a traffic ceiling, an outright hotlink ban is possible at any traffic level**: Minimal used to be Wikimedia's tile service until it 403'd in production (Wikimedia has blocked non-Wikimedia-project use since 2021), and CARTO's tiles (the original Streets/Minimal source) started requiring a paid key outright. Esri's two free layers (Minimal, Satellite) are the same risk class and could do the same. If real production traffic arrives, budget for a paid tile provider (MapTiler, Mapbox, Stadia Maps) to retire this risk in one move instead of firefighting each free provider as it breaks. |
