...
export default function AttractionSearch({ center, onSelect }) {
  ...
  useEffect(() => {
    ...
    // Contact fields (phone/hours) aren't part of a legacy Text Search result;
    // `choose` fetches those via Place Details only for the item picked.
  ...
  async function choose(place) {
    // Resolve a Google pick to its real coordinates and (for attractions)
    // contact details — the one Details call this specific pick costs.
    const resolved =
      place.source === "google" && place.placeId
        ? await resolveGooglePlace(place.placeId, { details: true }).catch(
            () => place,
          )
        : place;
